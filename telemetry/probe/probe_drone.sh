...
#
# Usage:  sh probe_drone.sh <cur_mcs> [link_id] [pps] [secs]
...
CUR_MCS="${1:?usage: probe_drone.sh <cur_mcs> [link_id] [pps] [secs]}"
LINK_ID="${2:-50}"
PPS="${3:-20}"
...
# rungs: current and current+1 (boundary probe). Edit to add +2 as confirmation.
RUNGS="$CUR_MCS $((CUR_MCS + 1))"
